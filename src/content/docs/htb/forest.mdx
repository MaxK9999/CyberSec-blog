...
==Quick scan==
First of all we'll do a quick scan to see what ports are open:

```bash 
┌──(kali㉿kali)-[~]
└─$ nmap 10.10.10.161                                           
Starting Nmap 7.94SVN ( https://nmap.org ) at 2024-08-04 09:25 IST
...
Now that we've found the general open ports we'll do a more thorough scan for good measure as well. While we do that we can already start looking up some of these ports to find out what they are. 
...
Starting Nmap 7.94SVN ( https://nmap.org ) at 2024-08-04 09:27 IST
...
Discovered open port 49668/tcp on 10.10.10.161
...
Completed Connect Scan at 09:27, 16.00s elapsed (65535 total ports)
Initiating Service scan at 09:27
Scanning 22 services on 10.10.10.161
Completed Service scan at 09:28, 53.73s elapsed (22 services on 1 host)
NSE: Script scanning 10.10.10.161.
NSE: Starting runlevel 1 (of 3) scan.
Initiating NSE at 09:28
Completed NSE at 09:28, 9.18s elapsed
NSE: Starting runlevel 2 (of 3) scan.
Initiating NSE at 09:28
Completed NSE at 09:28, 0.32s elapsed
NSE: Starting runlevel 3 (of 3) scan.
Initiating NSE at 09:28
Completed NSE at 09:28, 0.00s elapsed
Nmap scan report for 10.10.10.161
Host is up, received user-set (0.068s latency).
Scanned at 2024-08-04 09:27:14 IST for 80s
...
==MSRPC==
We see a shitload of `MSRPC` ports, but what does this mean? 
According to [hacktricks](https://book.hacktricks.xyz/network-services-pentesting/135-pentesting-msrpc) it means:
...
Since `135 / 139 and 445` are all open, I guess we'll have to find out which one will be of use to us (if not all of them). 
